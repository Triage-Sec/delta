...
class CompressionQualityPredictor:
    ...
    def _extract_features(
        ...
        features["dict_overhead_ratio"] = dict_len / compressed_len if compressed_len > 0 else 0
        ...
        features["original_diversity"] = unique_original / original_len if original_len > 0 else 1.0
        features["compressed_diversity"] = unique_compressed / compressed_len if compressed_len > 0 else 1.0
        features["diversity_change"] = (
            features["compressed_diversity"] / features["original_diversity"]
            if features["original_diversity"] > 0 else 1.0
        )
        ...
        features["coverage_ratio"] = total_replaced / original_len if original_len > 0 else 0
    ...
    def _generate_recommendation(
        ...
        current_ratio = result.compressed_length / result.original_length if result.original_length > 0 else 1.0
...
def evaluate_compression_quality(
    ...
    features["effective_compression"] = (
        ...
        if features["compression_ratio"] < 1.0 else 0.0
    )
    
    features["efficiency"] = (
        features["effective_compression"] / features["dict_overhead_ratio"]
        if features["dict_overhead_ratio"] > 0 else 0.0
    )
