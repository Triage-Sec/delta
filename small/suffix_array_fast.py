...
def benchmark_suffix_array(tokens: Sequence[Token], iterations: int = 5) -> dict[str, float]:
    ...
    return {
        ...
        "speedup": (sum(times_standard) / len(times_standard)) / (sum(times_fast) / len(times_fast)) if times_fast else 0,
        "n": len(tokens),
    }
