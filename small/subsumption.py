        ...
        for j in graph.subsumed_by[i]:
            ...
            for pos in subsuming_cand.positions:
                ...
                for offset in range(subsuming_len - cand_len + 1):
                    if subsuming_cand.subsequence[offset:offset + cand_len] == cand.subsequence:
                        subsuming_positions.add(pos + offset)
        
        # Find independent positions
        independent_positions = [p for p in cand.positions if p not in subsuming_positions]
        
        if len(independent_positions) >= min_independent_occurrences:
            # Check if independent occurrences alone are compressible
            if is_compressible(len(cand.subsequence), len(independent_positions), extra_cost):
...
def compute_coverage_score(
    ...
    for i, cand in enumerate(candidates):
        ...
        coverage[i] = min(1.0, total_covered / sequence_length) if sequence_length > 0 else 0.0
...
def rank_by_independent_value(
    ...
    for i, cand in enumerate(candidates):
        ...
        if graph.subsumed_by[i]:
            ...
            coverage_ratio = len(covered_positions & set(cand.positions)) / count if count > 0 else 0
            subsumption_penalty = base_savings * coverage_ratio * 0.8  # 80% penalty for covered positions
        ...
        final_score = base_savings - subsumption_penalty + maximal_bonus + subsumes_bonus
